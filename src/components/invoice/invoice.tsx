...
export default function Invoice({ dataInvoice }: { dataInvoice: object }) {
  // فلترة البيانات لكي تتناسب مع الجدول
  const itemsData = dataInvoice.Items.map((item: object) => ({
    ...
    total_price: truncateToTwoDecimals(
      item.item_many_invoice.price * item.item_many_invoice.quantity,
    ),
  }));
    ...
    <div className="Invoice">
      ...
      <div className="Invoice-info">
        ...
        <p className="one-info">
          {" "}
          عدد القطع :{" "}
          {dataInvoice.Items.reduce(
            (acc, item) => acc + item.item_many_invoice.quantity,
            0,
          )}
        </p>
        <p className="one-info">
          سعر الإجمالي : {truncateToTwoDecimals(dataInvoice.total_price)}
        </p>
        <p className="one-info">المبلغ المدفوع : {dataInvoice.paid_amount}</p>
        <p className="one-info">
          حالة التسديد وقت الشراء : {dataInvoice.payment_status}
        </p>
        <p className="one-info">
          تاريخ الفاتورة : {formatDateTime(dataInvoice.createdAt)}
        </p>
