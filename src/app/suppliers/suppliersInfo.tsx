...
export default function SuppliersInfo() {
  ...
  const { setWarningFunction, setWarningCardVisible, setWarningCardMessage } =
  ...
  async function sendPaymentSupplier() {
    setIsLoading(true);
    const response = await sendPaymentSupplierApi(
      selectedSupplierID,
      sendPaymentData,
    );
    if (response.success) {
  ...
  return (
    <div className="Suppliers-info">
      ...
      <div className="Suppliers-Info-Control">
        ...
        ></ReceiptLongIcon> */}
        <AttachMoneyIcon
          className={suppliersPaymentsListVisible === true ? "active" : ""}
          onClick={() =>
      ...
      <div className="Suppliers-info-content">
        ...
        {selectedSupplier.isUpdated && (
          <p>تاريخ اخر تعديل : {formatDateTime(selectedSupplier.updatedAt)}</p>
        )}
        <p>
          المستحقات : {truncateToTwoDecimals(selectedSupplier.payable_amount)} $
        </p>
      </div>
      ...
      <div className="Suppliers-info-inputs">
        <MyInput
          type_v="number"
          label_v={"المبلغ $"}
          input_v={sendPaymentData.payable_amount_send}
          onChange={(e) =>
            setSendPaymentData({
              ...sendPaymentData,
              payable_amount_send: Number(e.target.value),
            })
          }
        />
        <MyTextarea
          label_v={"ملاحظة"}
          data_v={sendPaymentData.note}
          onChange={(e) =>
            setSendPaymentData({ ...sendPaymentData, note: e.target.value })
          }
        />
      </div>
