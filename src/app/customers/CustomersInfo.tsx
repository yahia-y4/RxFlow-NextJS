...
export default function CustomersInfo() {
  ...
  const { setWarningFunction, setWarningCardMessage, setWarningCardVisible } =
    useContext(WarningContext);

  const {
    setCustomerPaymentsReceivedListVisible,
  ...
  async function handleDebtFormSubmit() {
    ...
    if (debtState === "Adding") {
      ...
      const result = await receivePaymentCustomerApi(
        selectedCustomer.id,
        formData,
      );
      if (result.success) {
  ...
  return (
    <div className="customers-info">
      ...
      <div className="customers-info-control">
        ...
        {/* الديون */}
        <AutoStoriesIcon
          className={CustomerDebtsListVisible === true ? "active" : ""}
          onClick={() => setCustomerDebtsListVisible(!CustomerDebtsListVisible)}
          style={{ fontSize: "30px", cursor: "pointer" }}
        />
        {/* الدفعات المستلمة */}
        <ChecklistRtlIcon
          className={
            CustomerPaymentsReceivedListVisible === true ? "active" : ""
          }
          onClick={() =>
            setCustomerPaymentsReceivedListVisible(
              !CustomerPaymentsReceivedListVisible,
      ...
      <div className="customers-info-content">
        ...
        <p className="one-customer-info">
          اسم الزبون : {selectedCustomer?.name}
        </p>
        <p className="one-customer-info">
          رقم الهاتف : {selectedCustomer?.phone_number}
        </p>
        <p className="one-customer-info">
          العنوان : {selectedCustomer?.address}
        </p>
        <p className="one-customer-info">
          تاريخ الاضافة : {formatDateTime(selectedCustomer?.createdAt)}
        </p>
        {selectedCustomer.isUpdated && (
          <p className="one-customer-info">{"معدل"}</p>
        )}

        {selectedCustomer.isUpdated && (
          <p className="one-customer-info">
            تاريخ التحديث : {formatDateTime(selectedCustomer.updatedAt)}
          </p>
        )}
        <p className="one-customer-info">
          مجموع الديون : {truncateToTwoDecimals(selectedCustomer?.debts)} $
        </p>
      </div>

      <div className="debt-control-buts">
        <TextButton
          selected={debtState === "Adding"}
          onClick={() => setDebtState("Adding")}
          text="اضافة دين"
        />
        <TextButton
          selected={debtState === "Receiving"}
          onClick={() => setDebtState("Receiving")}
          text="استلام دفعة"
        />
      </div>
      ...
      <div className="debt-inputs-div">
        ...
        />
        <MyTextarea
          data_v={formData.note}
          label_v="ملاحظة"
          onChange={handleNoteChange}
        />
        <div className="buts-div">
          <MyButton onClick={handleDebtFormSubmit}>
            {" "}
            {debtState === "Adding" ? "اضافة" : "استلام"}
          </MyButton>
